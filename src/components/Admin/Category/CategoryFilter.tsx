...
const CategoryFilter = (props: IProps) => {
    ...
    const onChangeStatus: GetProp<typeof Checkbox.Group, 'onChange'> = (checkedValues: any[]) => {
        const params = new URLSearchParams(searchParams.toString());
        if (checkedValues.length > 0 && checkedValues.length < STATUS_KEYS.length) {
                ...
                items={[
                    {
                        ...
                        children: (
                            <Checkbox.Group onChange={onChangeStatus} disabled={isErrorNetWork}>
                                <Row>
