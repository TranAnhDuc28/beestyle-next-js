...
interface HandleCartContextType {
    ...
    handleDeleteOrderItemCart: (id: number) => void;
...
const SaleComponent: React.FC = () => {
    ...
    const {handleCreateOrderItem, handleUpdateOrderItem} = useOrderItem();
    ...
    const handleDeleteOrderItemCart = useCallback((id: number) => {
...
const add = async () => {
    ...
    const result = await handleCreateOrder(defaultOrderPending);
    if (result) {
        // console.log(result);
...
return (
    <Layout className="h-screen" style={{background: colorBgContainer}}>
        <HandleCart.Provider
            value={{orderActiveTabKey, dataCart, setDataCart, handleDeleteOrderItemCart, handleAddOrderItemCart}}>
